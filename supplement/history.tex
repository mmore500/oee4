...
% In previous work with an earlier version of the system, we've seen multicellular traits evolve such as (blah from abstract) %doto
...
% Supplementary Figure \ref{fig:conflict:exactly_one} shows the  . %doto
...
% %doto change wording, not sure what you're hoping to say with first phrase
...
\subsection{Genotype Complexity}
\label{sec:supp-genotype-complexity}

Figure \ref{fig:complexity:genetic} plots genotype complexity of specimens drawn from across the case study's evolutionary history.

Genotype complexity reaches more than 20 under morph $b$, jumps to more than 40 under morph $d$, and drops to slightly more than 30 for morph $e$.
Genotype complexity reaches a peak of 52 sites around stint 39, then levels out and decreases.
...
To test whether increasing robustness accounted for this decrease, we ``skeletonized'' genomes by removing unexpressed sites then repeated our single-site knockout complexity assay.
...
No complexity crash, however, appears in assays configured with biotic background --- suggesting a contingent dependency to this background may have developed (Supplementary Figure \ref{fig:bb-complexity}).

Phylogenetic analysis (Supplementary Figure \ref{fig:phylo_parsimony_tree}) suggests independent origins of the genotype complexity in morph $d$ and morph $e$ --- the morph $d$ specimen from stint 14 is more closely related to the morph $b$ specimen from stint 13 than to the morph $e$ specimen from stint 15.
Likewise, specimens of lower-complexity morphs $i$ and $b$ sampled past stint 70 appear to have independent evolutionary origins.
...
Supplementary Figure \ref{fig:interface_complexity} summarizes phenotype complexity, as well as its constituent components, for specimens drawn from across the case study's evolutionary history.

Notably, relative to its stint 13 ancestor, cardinal processor phenotype complexity more than triples from 5 interactions to 17 interactions coincident with the emergence of morph $e$ at stint 15 (Supplementary Figure \ref{fig:interface_complexity:cardinal_interface_complexity}).
This is due to simultaneous increases in extrospective state sensing (0 to 4 states; Supplementary Figure \ref{fig:interface_complexity:extrospective_interface_complexity}) and introspective state sensing (1 to 9 states; Supplementary Figure \ref{fig:interface_complexity:introspective_interface_complexity}); writable state usage was unchanged at 2 states (Supplementary Figure \ref{fig:interface_complexity:writable_interface_complexity}).

The emergence of morph $g$ coincided with an increase in writable state phenotype complexity from 1 to 3, as shown in Supplementary Figure \ref{fig:interface_complexity:writable_interface_complexity}.
However, morph $g$ was not associated with changes in other aspects of cardinal processor phenotype complexity.
The greatest observed cardinal processor phenotype complexity was 22 interactions at stints 54 and 67.
...
The first observed morph $g$ specimen at stint 45 exhibited a relatively low active site count of around 100 sites.
...
\subsection{Cultured Phenotype Videos}
\label{sec:cultured-phenotype-videos}

To characterize evolved phenotypes, genomes collected at between-stint snapshots were cultured in separate simulation instances to create video animations.
For convenience, video from successive stints was concatenated to create a single ``montage'' video, with source stint of each video segment annotated.

Phenotype videos were taken for the following culture designs:
\begin{itemize}
\item \textbf{monoculture:} culture seeded from a single ``specimen'' genotype,
  \begin{itemize}
  \item case study replicate 16005 ``focal'' (cigar) strain specimen: \url{https://hopth.us/ib},
  \item case study replicate 16005 ``background'' strain specimen: \url{https://hopth.us/hw};
  \end{itemize}
\item \textbf{multiculture:} culture seeded from two ``specimen'' genotypes --- one each from focal and background strains,
  \begin{itemize}
  \item case study replicate 16005 specimens: \url{https://hopth.us/hy}
  \end{itemize}
\item \textbf{polyculture:} culture seeded from whole population, thus including both between- and within-strain standing variation,
  \begin{itemize}
  \item case study replicate 16005 population: \url{https://hopth.us/hi}; and
  \end{itemize}
\item \textbf{hemiculture:} culture seeded from whole population of a given strain, thus including within-strain standing variation,
  \begin{itemize}
  \item case study replicate 16005 ``focal'' (cigar) strain population: \url{https://hopth.us/ie},
  \item case study replicate 16005 ``background'' strain population: \url{https://hopth.us/if}.
  \end{itemize}
\end{itemize}

Cultures were performed under baseline conditions with mutation disabled.
In addition to hosted video links, raw video files are available with supplementary materials \citep{moreno2026osf,foster2017open}.
