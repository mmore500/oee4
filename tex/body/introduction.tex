...
Leveraging an \textit{in silico} approach to study synthetic instances of \textit{de novo} multicell evolution, we find direct evidence that spontaneous niche construction effects can stabilize substantially elevated genetic and phenotypic complexity.
Furthermore, we show, with much greater detail than previously possible, how this process unfolds in a concrete instance.
Critical to the work's tractability is our use of digital organisms as a model system: computer programs that replicate, mutate, compete, and thus evolve --- analogous to genomes within individual cells \citep{pennock2007models}.
...
Competition trials reveal that the high-complexity lineage evolved to become most fit when abundant --- in other words, when serving as its own environmental context.
Using further competitions, we next test for feedback between this niche construction effect and selection.
Lastly, we use replay experiments along this lineage's history to isolate the causal role of niche construction in maintaining complexity.
