...
% matches matplotlib's tab:cyan, used for morph j in the case-study plots
\definecolor{morphj}{HTML}{17BECF}

% Figure 2's case-study row sizes itself from its own contents rather
% than from the caption and strut lengths that vary between TeX
% distributions, so it keeps boxes for the side title, one panel picture,
% the whole panel row and a specimen subcaption, and measures those.
\newsavebox{\morphjtitlebox}
\newsavebox{\panelimgbox}
\newsavebox{\panelprobebox}
\newsavebox{\panelrowbox}
\newsavebox{\panelcapbox}
\newsavebox{\panelcapinkbox}
\newlength{\panelcolwd}
\newlength{\panelpad}
\newlength{\panelimght}
\newlength{\panelcapskip}
\newlength{\panelcapabove}
\newlength{\panelcapbelow}
\newlength{\panelcaptrim}
\newlength{\panelcappad}
\newlength{\panelrowht}
...
% Figures reference generated plot outputs (Jupyter binder teeplots) that
% live in sibling checkouts outside this repository. \allowmissingfigures
% is defined only by this repo's own Makefile (not by the paper
% superproject's), so a standalone build here tolerates a missing figure
% with a draft placeholder box, while the paper build still hard-fails if
% any of its pinned binder submodules are missing a referenced file.
\ifdefined\nofake
\ifdefined\allowmissingfigures
\makeatletter
\let\oee@includegraphics\includegraphics
\renewcommand{\includegraphics}[2][]{%
  \IfFileExists{#2}%
    {\oee@includegraphics[#1]{#2}}%
    {\PackageWarning{oee4}{Graphic `#2' not found; substituting draft placeholder}%
     \oee@includegraphics[#1,draft]{#2}}%
}
\makeatother
\fi
\fi
