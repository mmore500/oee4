...
Each cardinal processor has an independent copy of each piece of introspective state.
...
Each cardinal processor's introspective state is regularly copied and dispatched to that cardinal processor's neighbor cell, where it serves as read-only extrospective state.
...
That is, was the neighbor spawned from this cell, and is this cell older than the neighbor?
...
That is, are this cell's kin group ancestor ID(s) equal to the cardinal processor's neighbor's current kin group ID(s)?
...
Is the neighbor cell's kin group age greater than 80\% of this level's \texttt{GROUP\_EXPIRATION\_DURATIONS}?
...
That is, was the last cause of death on the current tile fragmentation?
...
Does this cell stockpile more resource than the cardinal processor's cell neighbor?
...
Number of CellAgeService calls elapsed since cell was born.
...
What was the most recent cause of death on this tile?
