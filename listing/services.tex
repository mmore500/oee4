...
They are then applied to another randomly-chosen cell in a \texttt{thread\_local} population, and another, until the entire population has been updated.
...
Executes a cell's genome on its cardinal processors for \texttt{HARDWARE\_EXECUTION\_CYCLES} virtual cycles.
...
Flushes each cardinal processor's inter-process and inter-thread output conduits.
...
(Depending on parameter settings, some cells may have no index set --- all positions within the bitstring are zeroed out.)

Broadcasts bits known to be set to all neighbor cells within the same kin group.
