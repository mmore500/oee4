...
In the operation descriptions below, we refer to register access via the $n$th argument as \texttt{reg[arg\_n]}.
...
Jumps the current core to a global anchor that matches the instruction tag if \texttt{reg[arg\_0]} is zero.
...
Sets the regulator value of local jump table tags matching this instruction's tag to \texttt{reg[arg\_0]}.
...
Adds \texttt{reg[arg\_0]} to the current value in a target writable state then stores the sum back into that target writable state.
...
Broadcasts this message to every other cardinal processor within the cell.

\subsection{Multiply Own State}

\opdef{5}{1}

Multiplies \texttt{reg[arg\_0]} by the current value in a target writable state then stores the result back into that target writable state.
