...
A few settings varied from run to run, and so are reported at their configuration file values: \texttt{STINT}, \texttt{SERIES}, and \texttt{TREATMENT} identify the individual run; \texttt{GENESIS} was ``generate'' for stint 0 and ``reconstitute'' thereafter; and \texttt{ALL\_DRAWINGS\_WRITE}, \texttt{RUNNINGLOGS\_DUMP}, and \texttt{CENSUS\_WRITE} were enabled on every tenth stint.
...
\confdef{uint64\_t}{4294967295}

How many evolutionary stints have elapsed?
Adjusted per transfer round.
