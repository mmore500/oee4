...
\newcommand{\confdef}[2]{
    ~\\ \begin{tabular}{|
        ...
        \hline
        Type & \texttt{#1} \\ \hline
        Setting   & \texttt{#2} \\ \hline
    \end{tabular}
}

This section enumerates simulation parameters and reports the settings used for evolutionary runs.
...
Settings reported here are taken from the \texttt{evolvejob} configuration file at \url{https://github.com/mmore500/dishtiny/tree/prq49/configpacks/bucket=prq49+diversity=0.50_series+mut_freq=1.00+mut_sever=1.00}; other assays (competition, monoculture, nopout, etc.) used equivalent settings defined in their own configuration files from the same configpack.

Where the evolution runscript (\url{https://github.com/mmore500/dishtiny/tree/prq49/slurm}) overrode a configuration file setting on the command line, the value reported below is the value as run.
In particular, population size was set via \texttt{WEAK\_SCALING} with \texttt{N\_CELLS} 3,600 per thread across 4 threads, giving the 14,400-cell population reported in the main text.
A few settings varied from run to run: \texttt{STINT}, \texttt{SERIES}, and \texttt{TREATMENT} identify the individual run, and are noted below as adjusted rather than given a value; \texttt{GENESIS} was ``generate'' for stint 0 and ``reconstitute'' thereafter; and \texttt{ALL\_DRAWINGS\_WRITE}, \texttt{RUNNINGLOGS\_DUMP}, and \texttt{CENSUS\_WRITE} were enabled on every tenth stint.

Three array-valued parameter types appear below, indexed by kin group level (\texttt{DISH2\_NLEV}, which is $L=2$ in this work):
\begin{itemize}
\item \texttt{internal::nlev\_float\_t} is \texttt{emp::array<float, DISH2\_NLEV>};
\item \texttt{internal::nlev\_size\_t\_t} is \texttt{emp::array<size\_t, DISH2\_NLEV>}; and
\item \texttt{internal::nreplev\_float\_t} is \texttt{emp::array<float, DISH2\_NLEV + 1>}, carrying one extra entry for offspring sharing no kin group with the parent.
\end{itemize}
...
Not triggered during experiments.
...
\confdef{uint64\_t}{std::numeric\_limits<uint64\_t>::max()}
...
\confdef{uint64\_t}{adjusted per transfer round}

How many evolutionary stints have elapsed?

\subsubsection{SERIES}

\confdef{uint64\_t}{adjusted per replicate}
...
\confdef{internal::nlev\_size\_t\_t}{[ 256 1024 ]}
...
\confdef{internal::nlev\_float\_t}{[ 0.25 0.25 ]}

How much resource should cells accrue per update?

\subsubsection{OPTIMAL\_QUORUM\_COUNT}

\confdef{internal::nlev\_size\_t\_t}{[ 12 12 ]}
...
\confdef{internal::nlev\_float\_t}{[ 1 1 ]}
...
\confdef{internal::nreplev\_float\_t}{[ 0.1 0.1 1 ]}
...
At what rate should bits be flipped in writable memory?
...
\subsubsection{CELL\_AGE\_SERVICE\_FREQUENCY}
