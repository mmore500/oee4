...
Our investigation begins with a case study dissecting the evolution of a particularly complex multicell life history (Sections \labelcref{sec:case-study-lineage,sec:adaptation-to-coevolved-ecological-context,sec:biotic-context-influences-fitness,sec:case-study-strain-produces-biotic-selection,sec:self-acting-selection-maintains-complexity-in-case-study-strain}).
Then, motivated by this case study, we explore how biotic selection influences evolved complexity more generally across evolutionary trials (Sections \labelcref{sec:biotic-selection-drives-multicell-complexity,sec:biotic-selection-acts-between-coevolving-lineages}).

\subsection{Case-study Lineage}
\label{sec:case-study-lineage}

To screen structural and functional complexity across evolutionary replicates, we singled out distinct adaptations using genetic knockout and phenotypic interference competitions (Figures \labelcref{fig:16005-timelapse:genotype-complexity,fig:16005-timelapse:phenotype-complexity}, Section \ref{sec:measuringcomplexity}).
Among 40 replicates, an extreme outlier in resulting measures of genotype and phenotype complexity sustained in excess of 3$\times$ IQR across both dimensions (blue highlight, Figure \ref{fig:16005-timelapse:mean-complexity}).
...
\unskip\footnote{%
Descriptions of rarer morphs $c$, $d$, $f$, $h$, and $i$ appear in supplementary material.
Morph $a$ was assigned to an independent short-lived lineage.
}

Early sampled genomes mostly grew as unicells and small groups up to 4 cells (morph $b$; video \href{https://hopth.us/hj}{\texttt{hopth.us/hj}}).
%, although our earliest sample from the case study lineage formed oversized groups through unchecked cell proliferation (morph $c$, transfer round 2; \href{https://hopth.us/hk}).

The case study lineage's cigar-like patterning first arose at transfer round 15 (morph $e$; \href{https://hopth.us/hm}{\texttt{hopth.us/hm}}) and
...
secondary box-like vertical growth first arose at transfer round 45 (morph $g$; \href{https://hopth.us/ho}{\texttt{hopth.us/ho}}).
% However, around transfer round 60, cigar-like morph $e$ began to be sampled as a reversion from morph $g$.
Finally, at transfer round 100, we first observed morph $j$ (panels \labelcref{fig:16005-timelapse:update64,fig:16005-timelapse:update256,fig:16005-timelapse:update832,fig:16005-timelapse:update6720,fig:16005-timelapse:update7616}; \href{https://hopth.us/hr}{\texttt{hopth.us/hr}}), which began to produce cigar-like daughter propagules during the secondary growth phase.
...
We thus more densely sampled isolate assays to trace genotype and phenotype complexity at each transfer round (Figure \labelcref{fig:keyfig:gcomplexity,fig:keyfig:pcomplexity}, Supplementary Figure \ref{fig:complexity}).

In several instances, complexity measures increased abruptly with new morphology.
For instance, at transfer round 14, morph $d$ (irregular clumps of 10 to 15 cells) coincided with a spike in genotype complexity from 27 to 46 sites.
At transfer round 15, cigar-like morph $e$ grew phenotype complexity over threefold from 5 to 17 interactions.
However, in contrast to these abrupt innovations, genotype complexity grew in a more gradual phase after the rise of cigar-like morph $e$ --- from 28 to 52 sites between transfer rounds 16 and 39.
...
Split regression analysis suggests genotype complexity developed in two distinct regimes (Davies test, $p < 10^{-14}$, adj. $R^2=0.43$; Supplementary Figure \ref{fig:complexity-cryptic-regression:split}): increasing until transfer round 34 (Wald test, $R^2 = 0.59$, $p < 10^{-6}$), and then decreasing (Wald test, $R^2 = 0.24$, $p < 10^{-4}$) --- likely due to gains in mutational robustness masking knockout sensitivity (Supplementary Section \ref{sec:supp-genotype-complexity}).
...
Later fitness gains became more sparse --- a notable exception, though, being a strong fitness gain with 500-site genome expansion at transfer round 93 (Figure \ref{fig:keyfig}).
...
To assess whether these apparent fitness losses could be an artifact of within-population variation, we repeated assays to compete whole-population samples rather than monoclonal isolates.
...
\label{sec:adaptation-to-coevolved-ecological-context}
...
However, strong evidence for biotic selection arose in 6 cases, where fitness losses became fitness gains in co-evolved context.
...
Based on competition outcomes, this comparator-based fitness measure ranged from 0 (all losses) to 1 (all wins) (Figure \ref{fig:fit:comparator-fitness}).

By this measuring stick, case study lineage fitness was volatile during early evolution, but settled near or below 0.5 from transfer round 30 onwards (solid line, Figure \ref{fig:fitness-background}) --- meaning it undercompeted most other replicates.
...
Fitness in co-evolved context closely tracked baseline fitness, with only transient influences on competitiveness against comparators.

These results --- in conjunction with reciprocal experiments testing how the case study strain influenced its co-evolved counterpart (Supplementary Figure \ref{fig:external-bb-reciporical}) --- suggest neither systematic cooperation nor antagonism.
Nevertheless, both strains did tend overall to gain modest competitive advantage against comparators when tested with their co-evolved partner present (both $p < 10^{-4}$, exact Binomial tests; Supplementary Figure \ref{fig:external-bb-reciporical}), consistent with a relationship of passive or by-product mutualism \citep{marchal2017passive,connor1995benefits}.
...
The strength of this effect --- and its co-origin with cigar-like morph $e$ --- led us to consider an alternate driver of complexity: self-acting biotic selection arising from the case study strain itself.
\unskip\footnote{%
To assess the plausibility of self-acting niche construction effects, we reviewed the spatial distribution of case study focal and background strains in co-culture, and found --- indeed --- that they rapidly segregate into distinct self-dense spatial pockets (Supplementary Table \ref{tab:multiculture_root_snapshots_by_stint}).
}
...
Seeing evidence that the case study lineage's biogenic structure exerted self-acting selection, we next turned to test whether this selection stabilized complexity.
...
Any remaining amount of the case study strain inherently introduces some degree of self-context.
...
Replays were compared between both a ``self context'' treatment and a ``co-evolved context'' treatment, with the former using intact case study lineage genomes as context (Figure \ref{fig:replay-background:design}).
...
Self context preserved the case study strain's cigar-like morphology in 51 of 69 mutagenized replays --- whereas co-evolved context exerted weaker selection, retaining cigar-like morphology in only 29 cases  (Fisher's exact, $p < 0.001$; Figure \ref{fig:replay-background:moderatedose}).
Thus, biotic selection exerted by the case study strain indeed acted to stabilize its own structural and functional complexity.
...
Under these conditions, complexity outcomes were indistinguishable between intact-self and co-evolved treatments (Figure \ref{fig:replay-background:highdose}).
Thus, the case study strain's biotic selection appears to act specifically in maintaining its own evolved complexity.
In other words, evolved traits acted specifically to ``lock in'' complexity, as opposed to simply cultivating generic conditions conducive to complexity (e.g., weakening selection).
...
To test whether analogous niche construction effects arose at all in other replicates, we screened for self-adaptation --- competitive advantage against cross-replicate comparators specific to self-dense context.
Such self-adaptation did arise in other instances, with robust evidence in at least 5/39 additional lineages (Mann-Whitney tests, $n=10$ paired observations, $\alpha = 0.05$; Supplementary Figures \labelcref{fig:external-bb-all,fig:general-supp:eco-vs-self}).

Seeing broader self-adaptation, we next tested whether it influenced evolved complexity.
Due to wide variation in genome size after 100 transfer rounds, we compared using phenotype complexity --- how many distinct cell I/O operations contribute to strain fitness, reflecting the richness of evolved cell-cell interactions.
...
Comparing fitness measures across our original $n=40$ replicates, we indeed found a more positive relationship between complexity and self-context fitness (binomial GEE, $p < 0.001$; $p = 0.003$ excluding case study replicate; Figure \ref{fig:general:gee-base}).
Direct regression analysis further supports a direct relationship between self-adaptation and phenotype complexity (Supplementary Figure \ref{fig:reg-self-adaptation}).
However, no such relationship arose between fitness and phenotype complexity (Supplementary Figures \labelcref{fig:complexity-fitness-correlation-genetic-9s:phenotype-unagg,fig:complexity-fitness-correlation-genetic-9s:phenotype-agg,fig:complexity-fitness-correlation-genetic:phenotype-unagg,fig:complexity-fitness-correlation-genetic:phenotype-agg}).
Complexity therefore reflects adaptation specific to biotic context, rather than generalized advantage in competitive ability --- consistent with niche construction acting as a selective ratchet for complex traits.
...
In contrast to 85\% of high-complexity strains, only 54\% of lower-complexity strains exhibited such self-advantage (Fisher's exact test, $p = 0.005$, RR 1.58; Figure \ref{fig:general:pgcomplex-ecoself}, right).

% 17028, 21036, 28020
Among screened high-complexity strains, only 3 of 20 exhibit cigar-like morphologies analogous to the case study strain.
Thus, while the case study's evolutionary trajectory appears qualitatively reproducible, it only accounts for a small fraction of possible high-complexity outcomes --- nearly all alternate morphologies exhibit niche construction effects (16/1 vs. 204/176; Fisher's exact test, $p = 0.001$, RR 1.75).
...
Alongside conspecific peers, natural multicells additionally encounter numerous heterospecific biota from co-evolving lineages.
To assess the role of such between-lineage interactions on multicell complexity, we next investigated whether selection between co-evolving lineages produced evolved complexity.
...
Thus, to begin, we examined whether --- in addition to selecting on itself --- the case study strain selected for genotype complexity in its co-evolved partner.

Under self-context conditions, knockouts on the co-evolved partner strain identified 17 sites as adaptive (Figure \ref{fig:ecogwas:sites}).
...
However, competition experiments also identified 13 further sites adaptive exclusively in the presence of the case study strain (Figure \ref{fig:ecogwas:sites}).
Thus, co-evolutionary interactions with the case study strain substantially drove evolved complexity in its co-evolved partner --- among 32 adaptive sites, 40\% arose by between-lineage selection.
...
Among sampled strains, adaptive sites specific to co-evolved context were detected in 6 of 7 exhibiting strong co-evolutionary adaptation --- more than 2 of 7 detected under weak co-evolutionary adaptation (Mann-Whitney test, $p=0.016$; Cliff's $\delta=0.76$; Figure \ref{fig:ecogwas:ythresh045}).
Thus, partner-specific selection appears to have additionally acted in creating evolved genotype complexity.

Taken together, reported results implicate biotic selection in general --- self-generated and co-evolved alike --- as a viable driver of multicellular complexity.
