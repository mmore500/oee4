\begin{figure*}

\input{fig/phylogeny/stint_roots.tex}~
\input{fig/phylogeny/lowestroot_stint_roots.tex}
\input{fig/phylogeny/updates_vs_stint_roots.tex}~
\input{fig/phylogeny/log_updates_vs_stint_roots.tex}

\caption{
\textbf{Phylogenetic statistics.}
\footnotesize
Color coding and letters correspond to qualitative morph codes described in Table \ref{tab:morph_descriptions}.
Dotted vertical line denotes emergence of morph $e$.
Dashed vertical line denotes emergence of morph $g$.
}
\label{fig:phylogeny}
\end{figure*}
