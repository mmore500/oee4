...
% Save the panel row so the band below the pictures can be measured
% instead of derived from \abovecaptionskip and the caption package's
% own skips, which differ between TeX distributions.
\savebox{\panelrowbox}{%
...
% a specimen subcaption, strutted so it measures the line the subcaption
% is set on rather than just the reach of its letters
\savebox{\panelcapbox}{\footnotesize\strut(h) update 952}
% Whatever the saved row holds below the pictures beyond that line is the
% gap above the subcaptions; leave the same amount underneath so they sit
% midway between the pictures and the bottom of the frame.
\setlength{\panelcapgap}{\dimexpr\ht\panelrowbox+\dp\panelrowbox
